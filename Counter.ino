...
//hstrings = Display.file_Open("COUNTE~1.txf", 'r') ;                            // Open handle to access uSD strings, uncomment if required

  // put your setup code here, to run once:

hndl = Display.file_LoadImageControl("COUNTD~1.dat", "COUNTD~1.gci", 1);// Winbutton1 show button, only do this once
...
if (state == TOUCH_PRESSED && n == iWinbutton1) {
  Display.img_SetWord(hndl, iWinbutton1, IMAGE_INDEX, 1); // Winbutton1 where state is 0 for up and 1 for down
  Display.img_Show(hndl,iWinbutton1) ;  // Winbutton1
if(value < 99){
value++;
}
  LedDigitsDisplay(Display, hndl, value, iLeddigits1+1, 60, 2, 1, 39, 0) ;  // Leddigits1
...
if (state == TOUCH_PRESSED && n == iWinbutton2) {
  Display.img_SetWord(hndl, iWinbutton2, IMAGE_INDEX, 1); // Winbutton2 where state is 0 for up and 1 for down
...
if (state == TOUCH_PRESSED && n == iWinbutton3) {
  Display.img_SetWord(hndl, iWinbutton3, IMAGE_INDEX, 1); // Winbutton3 where state is 0 for up and 1 for down
